...
void Reset_Handler(void) {
    ...
    uint32_t* data_flash = &__fsdata;
    uint32_t* data_ram = &__sdata;
    while (data_ram < &__edata) {
        *(data_ram++) = *(data_flash++);
    }
    /* set whole .bss section to 0 */
    uint32_t* bss = &__sbss;
    while (bss < &__ebss) {
        *(bss++) = 0;
    }
...
void (*const vectors[])(void) __attribute__ ((section(".isr_vectors"))) = {
        (void (*)(void))&__stack_end,           // Main stack end address
        Reset_Handler,                          // Reset
